####################################
#                                  #
#   GHG-Rechner für Weizenversuche   #
#                       #
#                                  #
####################################
...
GHGCalculator_W_4_1 <- function(Ertrag, pH=6.5,Treatment, Feuchte=13.5, Nmineral=114, SimNLeach=0, SimN2ON=0, SimResidue=0, sim=FALSE,
                                Norgan=0, P2O5=NULL, K2O=NULL, CaCO3=NULL, Dataset=NULL,
                                PSM=5.853 , Saatgut=36, Diesel=91.3, Field.acidification=NULL,
  ...
  Ref.BG.ef.Electricity <- 0.17031314                                      #BG II additional Standardvalues --> IFEU calculations
  ...
  Ref.FA.N.EPA <- 0.386                                 #  ((0.216 g CO2/g CaCO3) berechnet nach Cai et al.,2014 ## EPA(Modell GREET1_2014) 
  Ref.BG.conversion <- 0.5313
  Ref.BG.allocation <- 0.551
  Kalkung.kg_MJWeizen <- 0.00257 #(Edward et al. 2019, S.73)
  
  # Werte fuer direkte und indirekte N2O-Emission  -  maessten nach Biograce II ueber GNOC berechnet werden
  ...
  if(is.numeric(CaCO3)) {
    In.CaCO3 <- Out.Ertrag.Feucht * 0 # Duengung muss als Vektor
  ...
  # TotNInput_F <- In.F_SN+In.F_ON
  # names(TotNInput_F) <- "TotNInput_F"
  # TotNInput_F <-  ifelse(TotNInput_F == 0, 1, TotNInput_F)
